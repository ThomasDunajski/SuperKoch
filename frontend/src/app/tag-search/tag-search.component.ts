...
export class TagSearchComponent implements OnInit {
    ...
    this.activatedRoute.queryParams.subscribe(async (params) => {
      ...
      if (this.allTags && this.allTags.length > 0) {
        ...
        await this.getAllTags(
          this.activatedRoute.snapshot.queryParamMap.get('selectedTags')
        );
      }
  ...
  remove(tag: Tag): void {
    ...
    if (index >= 0) {
      ...
      this.tagCategorys = this.tagCategorys.map((category) =>
        category[0].category.name === tag.category.name
          ? category.concat(tag)
          : category
      );
      this.updateSearchParams();
  ...
  processTags() {
    ...
    this.allTags.forEach((tag) => {
      if (!categorys.hasOwnProperty(tag.category.number))
        categorys[tag.category.number] = [];
  ...
  getRecipeSearch = async function () {
    this.recepies = await this.api.getRecipeSearch(
      this.selected,
      this.searchName
    );
  };
