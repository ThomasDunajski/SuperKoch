...
export class CollectionComponent implements OnInit {
  ...
  updateSummary(){
    ...
    let recipes:Recipe[] = [];
    this.collection.sections.forEach(section => {
      recipes = recipes.concat(section.recipes.filter(recipe => this.selectedRecipeNumbers.indexOf(recipe.number) >= 0));
    });
    this.selectedRecipes = recipes;

    //update ingredients list
    let ingredients = [];
    recipes.forEach(recipe => ingredients = ingredients.concat(recipe.ingredients))
